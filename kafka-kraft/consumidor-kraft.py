#!/usr/bin/env python3
"""Consumidor Python para el stack Kafka KRaft."""
...
def create_consumer(bootstrap_servers, group_id=None):
    if group_id is None:
        group_id = f'consumer-kraft-{int(datetime.now().timestamp() * 1000)}'
...
def main():
    parser = argparse.ArgumentParser(description='Consumer para Kafka KRaft')
    parser.add_argument('--bootstrap-servers', default='localhost:51437', help='Broker Kafka KRaft (default: localhost:51437)')
    ...
    print('▶ Iniciando consumidor Kafka KRaft')
